 ...
 */
inline bool poseValidator(rawAnglesT &poseByAngles, const SUPPORT_FOOT &supFoot,
                          float &com2CentroidDist,
                          const SupportPolygon &supportPoly) {
  ...
  KinematicMatrix lFoot2torso = ForwardKinematics::getLFoot(
      rawPoseT(poseByAngles.begin() + JOINTS::L_HIP_YAW_PITCH,
               poseByAngles.begin() + JOINTS::L_ANKLE_ROLL + 1));
  KinematicMatrix rFoot2torso = ForwardKinematics::getRFoot(
      rawPoseT(poseByAngles.begin() + JOINTS::R_HIP_YAW_PITCH,
               poseByAngles.begin() + JOINTS::R_ANKLE_ROLL + 1));
  com2CentroidDist = 0;
  bool isStable = supportPoly.isComWithinSupport(
      lFoot2torso, rFoot2torso, com2torso, com2CentroidDist, supFoot);
  return isStable;
}

void jointIterFuncWithLim(
    const size_t torsoPosBegin, const size_t torsoPosEnd,
    const SUPPORT_FOOT supFoot,
    const std::vector<HeadYawPitch> headYawPitchList,
...
int main(int argc, char **argv) {
  ...
  size_t maxIterCount = headYawPitchList.size();
  {
    ...
    for (int i = static_cast<int>(paramNameT::P_TORSO_POS_X);
         i < static_cast<int>(paramNameT::P_MAX); i += 3) {
      minMaxInobj.getLimits(static_cast<paramNameT>(i), minLimit.x(),
                            maxLimit.x(), increment.x());
      minMaxInobj.getLimits(static_cast<paramNameT>(i + 1), minLimit.y(),
                            maxLimit.y(), increment.y());
      minMaxInobj.getLimits(static_cast<paramNameT>(i + 2), minLimit.z(),
                            maxLimit.z(), increment.z());
      switch (i) {
      case static_cast<int>(paramNameT::P_TORSO_POS_X):
        torsoPosList =
            PoseUtils::getVector3List<dataT>(minLimit, maxLimit, increment);
  ...
  /// Start the real threading..
  {
    ...
    for (unsigned int i = 0; i < THREADS_USED; i++) {
      poseAndAnglesFiles[i] = std::fstream(
          (outFileName + "_TempGeneratedPoses_" + std::to_string(i) + ".txt"),
          std::ios::out);
      ...
      // void jointIterFuncWithLim(const size_t torsoPosBegin, const size_t
      // torsoPosEnd, const SUPPORT_FOOT supFoot,
      //                           const std::vector<HeadYawPitch>
      //                           &headYawPitchList, const vector3ListT
      //                           torsoPosList, const vector3ListT
      //                           torsoRotList,
      //                           const vector3ListT OFootPosList,
      //                           const vector3ListT OFootRotList, poseListT
      //                           &poseList, std::ostream &poseOutStream,
      //                           const SupportPolygon &supportPoly,
      //                           std::atomic<size_t> &iterCount)
      threadList[i] = std::thread(
          jointIterFuncWithLim, start, end, supportFoot, headYawPitchList,
...
/// Write the remaining buffers to file
    ...
    for (size_t i = 0; i < THREADS_USED; i++) {
      utils::commitToStream<NaoPoseAndRawAngles<dataT>>(
          poseAndAnglesListList[i], poseAndAnglesFiles[i]);
    ...
    // redistribute the written lines.
    {
      ...
      std::fstream outStream =
          std::fstream((outFileName + "_GeneratedPoses_" +
      ...
      for (size_t i = 0; i < THREADS_USED; i++) {
        ...
        while (std::getline(inStream, s)) {
                                               ...
                                               // to the same file.
          {
            ...
            outStream.open((outFileName + "_GeneratedPoses_" +
                            std::to_string(outFileNum) + ".txt"),
                           std::ios::out);
          }
        }
  ...
  std::cout << "Tried " << std::scientific
            << static_cast<long double>(std::accumulate(
                   iterCount.begin(), iterCount.end(), static_cast<size_t>(0)))
            << " poses!" << std::endl;
  std::cout << "Found " << std::scientific
            << static_cast<long double>(poseCount.load()) << " good poses!"
            << std::endl;
