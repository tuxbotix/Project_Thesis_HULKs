...
public:
  Capsule(const float &radius)
      : radius(radius), length(0), begin(Vector3f::Zero()),
        end(Vector3f::Zero()) {}
  Capsule(const float &radius, const Vector3f &begin, const Vector3f &end)
      : radius(radius), length((begin - end).norm()), begin(begin), end(end) {}
...
public:
  const float radius;
  Vector3f origin;
  Sphere(const float &radius, const Vector3f &origin)
      : radius(radius), origin(origin) {}
  Sphere(const float &radius) : radius(radius), origin(Vector3f::Zero()) {}

  void update(const Vector3f &origin) { this->origin = origin; }
};
...
public:
   ...
   */
  CollisionModel()
      : lThigh(40), rThigh(40), lTibia(40), rTibia(40), lFootInnerEdge(20),
        rFootInnerEdge(20) {}
