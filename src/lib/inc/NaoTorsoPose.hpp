...
private:
  ...
  static bool leanToTorsoPose(jointAnglesT &jointAngles,
    ...
    if (supportFoot == SUPPORT_FOOT::SF_DOUBLE) {
      ...
      otherFoot2SF_copy.rotM =
          AngleAxisf(otherFootRot2SupportFoot.z(), Vector3f::UnitZ());
    } else {
      otherFoot2SF_copy.posV = otherFootPos2supportFoot;
      otherFoot2SF_copy.rotM =
          AngleAxisf(otherFootRot2SupportFoot.z(), Vector3f::UnitZ()) *
    ...
    KinematicMatrix otherFoot2targetTorso =
        supFoot2targetPose * otherFoot2SF_copy;
    ...
    if (supportFoot == SUPPORT_FOOT::SF_LEFT ||
        supportFoot == SUPPORT_FOOT::SF_DOUBLE) {
      lLegAngles = InverseKinematics::getLLegAngles(supFoot2targetPose);
      rLegAngles = InverseKinematics::getFixedRLegAngles(otherFoot2targetTorso,
                                                         lLegAngles[0]);

      genTorso2SupFoot = ForwardKinematics::getLFoot(lLegAngles).invert();
      errFootPos =
          (genTorso2SupFoot * ForwardKinematics::getRFoot(rLegAngles)).posV;
    } else {
      rLegAngles = InverseKinematics::getRLegAngles(supFoot2targetPose);
      lLegAngles = InverseKinematics::getFixedLLegAngles(otherFoot2targetTorso,
                                                         rLegAngles[0]);

      genTorso2SupFoot = ForwardKinematics::getRFoot(rLegAngles).invert();
      errFootPos =
          (genTorso2SupFoot * ForwardKinematics::getLFoot(lLegAngles)).posV;
    }
    /// Inv. Kinematics sometimes cannot reach a given pose, thus a nearby-enogh
    /// pose is given.
...
public:
  static bool getPose(jointAnglesT &jointAngles,
    ...
    return leanToTorsoPose(jointAngles, torsoPose,
                           ...
                           otherFootRot2SupportFoot, supportFoot);
  }
};
