...
float Capsule::getLength() const { return length; }

bool Capsule::pointInCapsuleCore(const Vector3f &pt) const {
  return (getLength() - (begin - pt).norm() - (end - pt).norm()) <
         __FLT_EPSILON__;
}
...
bool Capsule::isColliding(const Sphere &spr) {
  ...
  {
    return std::min((spr.origin - begin).norm(), (spr.origin - end).norm()) <
           radiusTotal;
  }
}
...
bool Capsule::isColliding(const Capsule &caps) {
  ...
  float dist = GeomUtils::closestPtRayToRay(begin, end, caps.begin, caps.end,
                                            closestPts);
  if (dist < __FLT_EPSILON__) {
    ...
    // both intersections are inside each capsule -> in 2D plane this is
    // intersection
    ...
    if (intersectInCaps1) {
      float minDist =
          std::min(GeomUtils::distPointToLine(begin, end, caps.begin),
                   GeomUtils::distPointToLine(begin, end, caps.end));
      return minDist < radiusTotal;
    } else if (intersectInCaps2) // caps 2
    {
      float minDist =
          std::min(GeomUtils::distPointToLine(caps.begin, caps.end, begin),
                   GeomUtils::distPointToLine(caps.begin, caps.end, end));
      return minDist < radiusTotal;
    } else // intersection is outside both
    {
      float minDist1 =
          std::min((begin - caps.begin).norm(), (end - caps.end).norm());
      float minDist2 =
          std::min((begin - caps.end).norm(), (end - caps.begin).norm());
...
void CollisionModel::updateModel(rawPoseT &jointAngles) {
  std::vector<KinematicMatrix> lLegJointPoses =
      ForwardKinematics::getLLeg(jointAngles);
  std::vector<KinematicMatrix> rLegJointPoses =
      ForwardKinematics::getRLeg(jointAngles);
