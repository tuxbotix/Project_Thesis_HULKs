...
double poseFilterCostFunc(const PoseSensitivity<T> &p, const float &wSensorMag,
                          const JointWeights &wJoints,
                          const float &wOrthoGeneric,
...
int main(int argc, char **argv) {
  ...
  /// Start the real threading..
  {
    ...
    std::cout << "total filtered pose count " << sortedPoseCostVec.size()
              << std::endl;
    // sort the results by ID. Will be expensive -_- But will save time in
    // lookup.
    //        std::sort(sortedPoseCostVec.begin(), sortedPoseCostVec.end(),
    //        [](PoseCost & a, PoseCost & b) {
    ...
    //        /// Sort filtered poses in cost order and commit cost and pose
    //        seperately.
    ...
    bool first = true;
    const size_t trimmedLen =
        sortedPoseCostVec.size(); // std::min(finalPoseFilteringLimit,
                                  // sortedPoseCostVec.size());

    std::cout << "starting nearest pose filtering" << std::endl;
    // Bit of a nearest neighbour..
    NaoPose<float> curTopPose = poseMap.at(sortedPoseCostVec[0].second).pose;
    HeadYawPitch hYPbounds(5, 1);
    float torsoPosBound = 2.8;
    float torsoRotBound = 5.1;

    size_t iter = 0;
    for (size_t i = 0; i < trimmedLen; i++) {
      auto pose = poseMap.at(sortedPoseCostVec[i].second);

      if (first ||
          !curTopPose.isNear(hYPbounds, torsoPosBound, torsoRotBound,
                             pose.pose)) {
        try {
          //                poseList[i].pose.
          outFilteredPosesFile << pose << std::endl;
          outCostsFile << "poseCost " << sortedPoseCostVec[i].second << " "
                       << sortedPoseCostVec[i].first << std::endl;
        } catch (std::exception e) {
          std::cerr << e.what() << " " << sortedPoseCostVec[i].second
                    << std::endl;
          return 1;
        }
        // update the current top pose
