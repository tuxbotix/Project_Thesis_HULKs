...
const int BUFFER_SIZE =
    totalBufferSizeInBytes / (sizeof(dataT) * JOINT_COUNT * MAX_THREADS);
...
inline void recursiveIter(const jointT &joint, dataT start, dataT end,
                          poseListT &poseList, poseT &accum,
                          std::ostream &outStream, const bool &inclusiveMax);
inline void recursiveIter(const jointT &joint, dataT start, dataT end,
                          poseListT &poseList, poseT &accum,
                          std::ostream &outStream, const bool &inclusiveMax) {
  ...
  for (dataT i = start; i < end || (inclusiveMax ? i == end : false); i++) {
    ...
    if (joint == 0) {
      ...
      std::cout << "iter " << inclusiveMax << " " << start << " " << end
                << std::endl;
    }
#endif
    accum[joint] = i;
    ...
    if (nextJoint == JOINT_COUNT) {
      ...
      recursiveIter(nextJoint, global_min, global_max, poseList, accum,
                    outStream, true);
    }
  }
}
...

  {
    ...
    for (unsigned int i = 0; i < THREADS_USED; i++) {
      ...
      threadList[i] = std::thread(
          recursiveIter, 0, start, end, std::ref(poseListList[i]),
