...
//    Eigen::VectorXf inducedErrorEig = Eigen::Map<Eigen::VectorXf,
//    Eigen::Unaligned>(inducedErrorStdVec.data(), inducedErrorStdVec.size());
...
//    std::cout<< "Without Calib state: " <<
//    calibrator(Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(calVec.data(),
//    calVec.size()), errorVec) << " cost: " << errorVec.norm() << std::endl;

//    Eigen::NumericalDiff<JointCalibrator> functorDiff(calibrator);
//    Eigen::LevenbergMarquardt<Eigen::NumericalDiff<JointCalibrator>, float>
//    lm(functorDiff);
...
//    std::cout<< "Calibrated status:: " << calibrator(calibratedParams,
//    finalErrorVec) << " cost: " << errorVec.norm() << std::endl;
...
//    calibratedResidualDump(finalErrorVec, inFileName + ".calibratedResidual");
